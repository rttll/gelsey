<template>
  <article :data-order="album.order" class="w-full" :class="classes">
    <div class="relative w-full album-container">
      ...
      <a
        v-if="album.link"
        :href="album.link"
        class="absolute bottom-0 right-0 z-50 inline-flex items-center p-4 space-x-1 text-white album-content"
      >
  ...
  data() {
    return {
      style: `background-image: url(${
        this.album.cover.asset.url
      }); background-size: cover`,
      classes: this.small
        ? 'album-md:w-1/2 md:w-1/4'
        : 'album-md:w-1/2 md:w-1/3',
      titleClasses: this.small
        ? 'album-md:text-xl md:text-2xl'
        : 'album-md:text-2xl md:text-3xl',
    };
  },
};
</script>
